...
def get_config():
    ...
    parser.add_argument(
        ...
        action="store_false",
        default=True,
        help="use reward shaping function in Sampling",
    )
    parser.add_argument(
        "--use_shaping_weight",
        action="store_false",
        default=True,
        help="shaping할 때 더하고 빼는 값에 가중치를 부여할 것인지",
    )
    parser.add_argument(
        "--shaping_weight_type",
        type=str,
        default="sigmoid_flipsigmoid",
        choices = ["sigmoid_flipsigmoid", "sigmoid"],
        help="use reward shaping function in Sampling",
    )

    # prepare parameters
    parser.add_argument(
        "--algorithm_name", type=str, default="rmappo", choices=["rmappo", "ippo", "mappo", "happo", "hatrpo", "mat", "mat_dec"]
    )
    parser.add_argument(
        "--group_name",
        type = str,
    ...
    parser.add_argument(
        "--experiment_name", type=str, default="check", help="an identifier to distinguish different experiment."
    )
    ...
    parser.add_argument(
        "--n_rollout_threads", type=int, default=1, help="Number of parallel envs for training rollouts"
    )
    parser.add_argument(
        "--n_eval_rollout_threads", type=int, default=1, help="Number of parallel envs for evaluating rollouts"
    )
    parser.add_argument(
    ...
    # network parameters
    parser.add_argument(
        "--share_policy", action="store_false", default=True, help="Whether agent share the same policy"
    )
    parser.add_argument(
        "--use_centralized_V", action="store_false", default=True, help="Whether to use centralized V function"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--data_chunk_length", type=int, default=10, help="Time length of chunks used to train a recurrent_policy"
    )
    ...
    # trpo parameters
    parser.add_argument(
        "--kl_threshold", type=float, default=0.01, help="the threshold of kl-divergence (default: 0.01)"
    )
    parser.add_argument("--ls_step", type=int, default=10, help="number of line search (default: 10)")
    ...
    # run parameters
    parser.add_argument(
        "--use_linear_lr_decay", action="store_false", default=True, help="use a linear schedule on the learning rate"
    )
    # save parameters
    parser.add_argument(
    ...
    # log parameters
    parser.add_argument(
        "--log_interval", type=int, default=10, help="time duration between contiunous twice log printing."
    )
    ...
    parser.add_argument(
        "--eval_interval", type=int, default=10, help="time duration between contiunous twice evaluation progress."
    )
    ...
    parser.add_argument(
        "--ifi", type=float, default=0.1, help="the play interval of each rendered image in saved video."
    )

    # pretrained parameters
    parser.add_argument(
        "--model_dir", type=str, default=None, help="by default None. set the path to pretrained model."
    )
