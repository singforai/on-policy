...
class SMACRunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                ...
                if self.use_reward_shaping:
                    if self.use_shaping_weight:
                        rewards, shaping_weight = self.reward_function.reward_shaping(
                            episode = episode, 
                            episodes = episodes, 
                            step = step, 
                        ...
                        rewards = self.reward_function.reward_shaping(
                            episode = episode, 
                            episodes = episodes, 
                            step = step, 
                            share_obs = torch.flatten(torch.tensor(share_obs)), 
            ...
            if episode % self.log_interval == 0:
                ...
                if self.use_reward_shaping:                
                    train_infos["reward_predicting_loss"] = episode_loss
                    if self.use_shaping_weight:
                        train_infos["shaping_weight"] = shaping_weight
    ...
    def warmup(self):
        ...
        if self.use_reward_shaping:
            self.reward_function = Reward_Function(share_obs = torch.flatten(torch.tensor(share_obs)), 
                                                   device = self.device, 
                                                   num_agent = self.num_agents, 
                                                   episode_length = self.episode_length,
                                                   use_shaping_weight = self.use_shaping_weight,
                                                   shaping_weight_type = self.all_args.shaping_weight_type,
                                                   )
